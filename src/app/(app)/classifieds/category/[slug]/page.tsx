...
function Avatar({ name, url }: { name?: string | null; url?: string | null }) {
  return url ? (
    ...
    <div className="w-9 h-9 rounded-full bg-[var(--bg-secondary)] border border-[var(--border)] flex items-center justify-center text-xs font-semibold text-[var(--primary)]">
      {avatarInitials(name)}
    </div>
  )
}
...
export default function ClassifiedCategoryPage() {
  ...
  useEffect(() => {
    async function load() {
      if (!section || section.slug === 'housing') {
        setLoading(false)
        return
      }
      const { data } = await supabase
        ...
        .in('categories.name', section.categoryNames)
        .eq('status', 'active')
          ...
          {housingSections.map(item => (
            <Link key={item.slug} href={`/classifieds/category/${item.slug}`} className="botanical-card overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-5">
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-1">View {item.label.toLowerCase()} posts →</p>
              </div>
            </Link>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="botanical-card p-10 text-center text-[var(--text-secondary)]">
...
function Breadcrumb({ label }: { label: string }) {
  return (
    <div className="text-sm text-[var(--text-secondary)]">
      <Link href="/" className="hover:text-[var(--primary)]">HOME</Link> <span>›</span> <span className="text-[var(--text)]">{label}</span>
    </div>
  )
}
...
function CategoryHero({ section }: { section: ReturnType<typeof getClassifiedSection> }) {
  ...
  return (
    <div className="botanical-card overflow-hidden">
      <div className="h-48 md:h-56 bg-[var(--bg-secondary)] flex items-center justify-center text-8xl" aria-label={`${section.label} category photo`}>
        {section.emoji}
      </div>
      <div className="p-5 text-center">
        <p className="text-sm text-[var(--text-secondary)]">KURIYA FRIENDS</p>
        <h1 className="text-3xl font-bold mt-1">{section.label}</h1>
      </div>
    </div>
  )
}
